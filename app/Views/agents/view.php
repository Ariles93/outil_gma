...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
    ...
    <?php if ($_SESSION['user_role'] === 'admin'): ?>
        <a href="<?= url('agents/edit?id=' . $agent['id']) ?>" class="btn btn-primary">Modifier</a>
    <?php endif; ?>
</div>
                            ...
                            <td class="actions-cell">
                                <form method="post" action="<?= url('assignments/return') ?>" class="return-form">
                                    ...
                                    <button type="submit" class="btn btn-success"
                                        style="padding: 0.4rem 0.8rem; font-size: 0.8rem;">Marquer comme retourné</button>
                                </form>
                            </td>
                        <?php endif; ?>
                    </tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (pdfToDownload) {
            // Note: Using the legacy script for now as PDF generation wasn't refactored

...
<div class="d-flex justify-between items-center mb-4">
    ...
    <a href="<?= url('agents/view?id=' . $agent['id']) ?>" class="btn btn-secondary">&larr; Retour</a>
</div>
...
<div class="card" style="max-width: 800px; margin: 0 auto;">
    ...
    <form method="post" action="<?= url('agents/edit?id=' . $agent['id']) ?>">
        ...
        <div class="form-group mt-4">
            ...
            <textarea id="notes" name="notes" rows="3"><?= e($agent['notes'] ?? '') ?></textarea>
        </div>

        <div class="d-flex justify-between items-center mt-6">
            <button type="submit" class="btn btn-primary">Mettre à jour</button>
            <a href="<?= url('agents/view?id=' . $agent['id']) ?>" class="text-muted">Annuler</a>
        </div>
    </form>
</div>
